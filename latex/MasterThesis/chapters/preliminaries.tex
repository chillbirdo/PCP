...
The $\alpha$ is called approximation ratio or performance guarantee of the $\alpha$-approximation algorithm. For minimization problems $\alpha > 1$ and for maximization problems $\alpha < 1$ holds. For example, a $1/2$-approximation algorithm for a maximization problem always returns a solution in polynomial time, that is at least half as good as the optimal solution. For some problem there even exist polynomial time algorithms, whose approximation ratio is given as parameter. They have so called polynomial-time approximation schemes.
...
A locally minimal solution (or local minimum) with respect to a neighborhood structure $\mathcal{N}$ is a solution $\hat{s}$ such that $\forall s \in \mathcal{N}(\hat{s}):f(\hat{s})\leq f(s)$. We call $\hat{s}$ a strict locally minimal solution if $f(\hat{s})<f(s) \mid \forall s \in \mathcal{N}(\hat{s})$.
\end{definition}
...
bly huge set of feasible solutions. 
