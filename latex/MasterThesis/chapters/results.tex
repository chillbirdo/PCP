...
To find out what method is more efficient to generate an initial solution, OneStepCD or DANGER combined with the prefixed node selection algorithm presented in chapter \ref{ch:approach}, tests have been performed on all instances. As mentioned above, one major disadvantage of DANGER is that a domain of colors has to be given as parameter and the algorithm then tries to color the graph only using colors in that domain. So in order to find out the lower bound of the domain, several runs have to be performed, which multiplies the runtime. Table \ref{tab:TODO}
